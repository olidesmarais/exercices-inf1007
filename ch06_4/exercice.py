...
def get_even_keys(dictionary):
	return set(cle for cle in dictionary if cle % 2 == 0)

def join_dictionaries(dictionaries):
	return {
		key: value
			for dictionnaire in dictionaries
...
def get_sum_values_from_key(dictionnaries, key):
	return sum(dictionnaire[key] for dictionnaire in dictionnaries if key in dictionnaire)
